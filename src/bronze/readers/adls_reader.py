...
adls_reader.py
--------------
Reads files from Azure Data Lake Storage Gen2 into Bronze-ready DataFrames.
...
def read_from_adls(
    ...
    """
    Read a file from ADLS Gen2.

    Args:
        spark: Active SparkSession
        uri:   Full abfss:// URI to the file
        job:   JobConfig for this dataset (drives format options)
...
def _split_and_cast(raw_df: DataFrame) -> tuple[DataFrame, DataFrame]:
    """Split clean vs corrupt rows and cast clean rows to STRING."""
...
def _cast_all_to_string(df: DataFrame) -> DataFrame:
    """Cast every column to STRING. Nested types become JSON strings."""
